...
def gene_molecule(num_H):
    ...
    molecule_data = MolecularData(geometry, basis, multiplicity=int(2 * spin + 1))
    molecule_data = run_pyscf(molecule_data, run_scf=1, run_ccsd=1, run_fci=1, verbose=False)
...
if __name__ == "__main__":
    ...
    _, _, _, ham_qubitOp, num_qubits, n_electrons = generate_uccsd(molecule_data)
    ...
    params_pool = []
    for k in range(100):
        x0 = np.random.randn(len(ansatz.params_name))*np.pi
    ...
    params_pool = np.load("pool2_normal.npy", allow_pickle=True)
